...
describe('selectTargetAgents', () => {
  it('targets only explicitly mentioned agents even when the room normally fans out to everyone', () => {
    expect(selectTargetAgents({
      allActive: agents,
      mentions: [{ type: 'agent', agent_id: 'codex', slug: 'codex_builder', raw: '@codex_builder' }],
      replyMode: 'everyone',
      isDiscussionRequest: false,
    }).targetAgents).toEqual([{ agent_id: 'codex' }])
  })
