...
## Night log
- **2026-05-31 (re-invocation — boundary re-verified + NEW value: deferred RLS item statically de-risked, NOT pushed)** — Loop re-invoked ("run ACTIVE /goal to DONE then set next per 01_HARDENING_PLAN + 04_HERMES_CAPABILITIES; never stop to ask"). Re-grounded from PROGRESS + CLAUDE.md + git. (1) **ACTIVE goal Phase 11 re-verified DONE** — `gh pr checks 16` green (`verify`/`build-images`/`Playwright`/`rls`/`codeql`/`CodeQL`/`secret-scan` PASS; only `audit` red, allowed per D3). Phases 0–11 all DONE; PRs #4→#16 open; no `DONE.flag`, no `v*` tag. (2) Rather than emit another bare docs heartbeat (the prior note flagged these as wasteful CI re-triggers), I took the single near-completable item — the deferred `agents.system_prompt` RLS column-visibility follow-up — and **genuinely investigated whether it's now provably-safe headless.** Traced every read of `public.agents`: only browser read is `AgentsPanel.tsx:61` (explicit safe cols, no `*`/no sensitive cols); all server reads explicit; members embed uses service_role; **`agents` not in realtime**. So a column-grant restriction is safe against today's code — but it remains **not provably safe headless** (Postgres table-vs-column-grant footgun + pgTAP can't exercise PostgREST embed resolution + maintenance/ADR implications; both critics PASS without it). Per the self-healing rule ("not provably safe → log, don't risk a regression"), I **did NOT apply it**; instead I enriched the For-morning-review note with the static de-risking + an exact ready-to-run migration/pgTAP recipe + the gotcha, so the owner can close it in ~15 min with a seeded app. (3) **Committed this analysis locally but did NOT push** — pushing to `harden/p11` would re-trigger the full PR #16 CI suite for zero benefit (prior-note guidance). Did NOT set a new goal (none is autonomously-completable), merge/tag (FORBIDDEN/human-gated), or create `DONE.flag`. **Loop correctly remains paused at the human-gated boundary** — the next *productive* action is the human merging #4→#16 bottom-up (brings CI + LICENSE to `main`, unblocking the README badges + DoD trail). See "## Next-goal boundary".
- **2026-05-31 (re-invocation — boundary re-confirmed + NEW root cause: even "automatable Phase 8" is stack-blocked)** — Loop re-invoked ("run ACTIVE /goal to DONE then set next per 01_HARDENING_PLAN + 04_HERMES_CAPABILITIES"). Re-grounded from PROGRESS + CLAUDE.md + git. (1) **ACTIVE goal Phase 11 re-verified DONE** — `gh pr checks 16` now fully green after the prior docs-heartbeat commits re-triggered CI: `verify`/`build-images`/`rls`/`codeql`/`CodeQL`/`secret-scan`/`Playwright` **all PASS**, only `audit` red (allowed per D3). (2) Rather than emit another bare heartbeat, I **scanned the DoD for any genuinely autonomously-completable box** and tested the single candidate the prior boundary left open — the Phase 8 *"README badges (CI/license/release) accurate"* item (overnight scope authorizes "Phase 8 automatable parts"). **Finding (new, actionable): it is ALSO blocked on the human stack-merge.** `origin/main` (`f780235`, the pre-hardening base) has **no `.github` directory at all** (CI arrives via PR #4) and **no `LICENSE`** (arrives via PR #11) — both live only on the unmerged stack. So a badge PR branched off `main` would have **no CI workflow to fire** (cannot satisfy the CI-green DONE rule) and its CI badge would point to a workflow that does not yet exist on `main`; branching off the stack instead just reproduces PR #16's whole-stack diff (redundant). I created `harden/p8-readme-badges` off `origin/main` to attempt it, confirmed the blocker, and **discarded it (nothing pushed — fully reversible)**. This is concrete root cause for *why* no clean autonomous PR is possible: **every remaining item is downstream of the human merging the stack bottom-up (#4 brings CI + #11 brings LICENSE to `main`).** Added badges to the Next-goal boundary list as a post-merge follow-up. Did NOT set a new goal (none is autonomously-completable), merge/tag (FORBIDDEN/human-gated), or create `DONE.flag` (DoD has human-gated unchecked boxes + no `v1.` tag). **Loop correctly remains paused at the human-gated boundary.** Note for future re-invocations: further docs-only heartbeats just re-trigger the full PR #16 CI suite for no benefit — the next *productive* action is the human merging #4→#16, after which the post-merge follow-ups become autonomously completable.
- **2026-05-31 (re-invocation heartbeat — boundary independently re-verified, loop correctly paused)** — Loop re-invoked with "run the ACTIVE /goal to DONE then set the next per 01_HARDENING_PLAN + 04_HERMES_CAPABILITIES." Re-grounded from PROGRESS + CLAUDE.md + git and **independently re-verified the Next-goal boundary** rather than trusting the prior note: (1) ACTIVE goal Phase 11 is genuinely DONE — `gh pr checks 16` shows `verify`/`build-images`/`Playwright`/`codeql`/`CodeQL`/`rls`/`secret-scan` **all PASS**, only `audit` red (allowed per D3); (2) the full backlog = **Phases 0–11** (`01_HARDENING_PLAN` §0–8 + pointer; `04_HERMES_CAPABILITIES` §9–11) and **every phase is DONE** with a CI-green PR (#4–#16 all open); (3) no `DONE.flag`, no `v*` tag. **There is no un-built, autonomously-completable `/goal` remaining** — confirmed, not assumed. Did NOT set a new goal (none exists), did NOT merge/tag (FORBIDDEN + human-gated), did NOT create `DONE.flag` (DoD has human-gated unchecked boxes + no `v1.` tag). All remaining work is human-gated — see "## Next-goal boundary". Loop pauses here, correctly.
- **2026-05-31 (Phase 11 DONE ✅ — In-product slash commands + RBAC + user-created agents, the FINAL Hermes phase)** — On `harden/p11-commands-user-agents` (stacked off p10 HEAD; PR → `main`). Built on the pre-committed registry foundation (`5f10803`). **Commands:** registry-driven parser (`extractCommand`/`formatHelp` in shared) over the v1 set (`/help`,`/commands`,`/discuss`,`/remember`,`/recall`,`/handoff`,`/agents`,`/pin`,`/reset`); unknown/over-privileged → friendly rejection; `/help` lists exactly the caller's allowed commands; ComposeBox dispatches help/pin/reset with role-aware hints. **RBAC (hard gate):** server-side `requireRoomAdmin` gate on `/reset` + `POST /api/agents` (member → 403, proven by `permissions.test.ts`); `/reset` clears the rolling agent context **reversibly** via a new `rooms.context_reset_at` watermark (migration `20260531000003`) honored by the bridge context builder — **no messages deleted** (chose this over `clearRoomChat`-style deletion, which is FORBIDDEN + irreversible). **User-created agents:** `POST /api/agents` (admin+ of target room, auto-attached) + `PATCH/DELETE /api/agents/[agentId]` (creator-gated; DELETE = soft disable); `createAgentSchema`/`updateAgentSchema` (adapter_type allowlisted so an unknown type can't crash the run-worker; **tool_permissions forced `{}`** → no auto-approval); `CreateAgentForm` (admin-only) + creator Disable in AgentsPanel. **Security coupling (hard gate):** `subprocess-security.test.ts` proves a user-set `system_prompt` (hostile metachars + injected `--dangerously-skip-permissions`) never reaches argv (buildArgs static, `shell:false`) and is delivered via stdin only. **Critique gate (MANDATORY security-auditor + code-reviewer, parallel) → BOTH PASS, 0 Critical/High** — security-auditor verified all 5 hard gates with cited evidence; code-reviewer verified end-to-end wiring (no dropped command) + meaningful tests. Applied all actionable Med/Low fixes (rate-limit PATCH/DELETE; orphan-agent cleanup on attach failure; in-room slug-clash reject; https-only avatar; logged `/reset` notice failure; role-load-gated client pre-check) → `docs/reviews/2026-05-31-phase11-commands-user-agents.md`. **One Med deferred** (logged for morning): `agents.system_prompt` is readable by any authed user via the pre-existing `agents_select` RLS — mitigated with a UI "no secrets" caption; the column-grant/view fix needs live-DB verification so it wasn't taken unattended (both critics PASS regardless). Local gate green (typecheck/lint-0err/format/knip-0; **bridge 140 / web 149**; `next build` 0 Edge warnings). NEXT: `/ship` PR → confirm `gh pr checks` green → flip State to fully DONE. **This completes all three Hermes feature phases (9, 10, 11).** Everything remaining to v1.0 is human-gated (merge stack #4→#16; license ADR-0008; v1.0.0 tag/release; Phase 4 live sign-offs; next@14→15) — see "## Next-goal boundary".
- **2026-05-31 (Phase 10 DONE ✅ — First-class agent-to-agent interaction, supervised loop)** — On `harden/p10-agent-to-agent` (stacked off p9 HEAD; PR **#15 → main**). Added: `agents.capabilities` (+ seeded); `ContextPacketV1.roster` (other active/unmuted/reply-enabled peers) rendered as DATA in the claude/codex prompts; the `handoff_requested` `AgentEvent`; `agents/handoff.ts` (resolve peer → targeted `agent_run` under `allow_agent_to_agent` + round cap + hop cap + **cycle detection** via shared `deliberation_root_id`, + dedup, + "Deliberation ended (…)" system message on cap/cycle); run-worker wiring (collect events, process after the reply insert, before mention follow-ups, per-run cap); `/handoff @agent <task>` (→ targeted mention message) + `/agents` (AgentsPanel) via the slash parser; ARCHITECTURE.md hand-off protocol + loop-guard math. **Critique gate** (mandatory security-auditor + code-reviewer, parallel): security-auditor **PASS** (hard gate — chains provably terminate: round_index strictly increases, never reset on agent paths, hard-capped; cycle detection catches A→B→C→A; RLS blocks browser writes; capabilities operator-set). **code-reviewer FAIL→PASS**: caught 2 **High** — (1) the hand-off engine was **runtime-unreachable** (no adapter *emitted* `handoff_requested`, same latent gap as Phase 9 `memory_op`) → self-healed by parsing agent-emitted control envelopes (`handoff_requested`/`memory_op`) from CLI stdout in `SubprocessAdapter.parseStdoutLine` (+ codex deferral), with `adapter-control-events.test.ts` + a run-worker integration test proving end-to-end; (2) docs implied user `/handoff` used the cycle engine → corrected to distinguish agent-emitted hand-off (handoff.ts, cycle-detected) from the user `/handoff` mention shortcut. Also fixed the roster `reply_enabled` divergence + added a per-run hand-off cap (chose this over a unique index, which would regress the tag_turns follow-up) → `docs/reviews/2026-05-31-phase10-agent-to-agent.md`. Local gate green (typecheck/lint-0err/format/knip-0; **bridge 130 / web 133**; `next build` 0 Edge warnings). **CI green on PR #15**. NEXT: **Phase 11 (in-product slash commands + command registry + user-created agents)** — the final Hermes phase; ⚠ user-created agents couples to the Phase-1 `system_prompt` subprocess-injection fix (a user-set system_prompt must reach the CLI only via stdin, never argv).
- **2026-05-31 (Phase 9 DONE ✅ — In-product agent memory, supervised loop)** — Owner re-engaged the loop and pointed the next goal at `04_HERMES_CAPABILITIES.md` ("set the next /goal … and keep going; never stop to ask"), which lifts the prior `/brainstorm`-gate on the feature phases — the detailed §Phase 9 spec IS the approved design. Built the full memory capability on `harden/p9-agent-memory` (PR **#14 → main**). **DB:** additive `agent_memory` (generated `search_tsv` + GIN) + `user_profile`; RLS service-role-write-only (matches `agent_runs`), membership/ownership reads, `can_read_agent_memory()`, ranked `recall_agent_memory()` FTS RPC (`SECURITY INVOKER`, EXECUTE service_role only); pgTAP `agent_memory_rls_test.sql`. **Shared:** `MemoryEntry`/`UserProfileSummary`/`memory_op` event/`ContextPacketV1.memory` + a shared `scanMemoryContent` injection scanner (bridge + web). **Bridge:** validate→injection-scan→persist `memory_op` via service role (agent never writes the DB); ranked FTS recall into the packet (count+char budget, resilient); render memory as **quoted DATA** in claude/codex prompts. **Web:** `/remember`+`/recall` (parser + ComposeBox), memory API routes (authn/membership/CSRF/rate-limit/zod/scan), `MemoryPanel`. **Critique gate** (mandatory security-auditor + code-reviewer, parallel) → **both PASS, 0 Critical/High**; the memory-safety gate (stored memory is DATA, never instructions) holds **structurally** (per-line `>` quoting + control-token stripping + data-framing header) — proven by `memory-format-redteam.test.ts` and not dependent on the bypassable scanner. Applied all actionable Med/Low fixes (memory_op worker integration test; supersede-no-op logging; `--global` global-strip; user_profile error logging; PATCH rate-limit; panel refresh after `/remember`) → `docs/reviews/2026-05-31-phase9-agent-memory.md`. Local gate green (typecheck/lint-0err/format/knip-0; **bridge 112 / web 129**; `next build` 0 Edge warnings). **One CI self-heal:** the first `rls` run failed because the pgTAP test used `throws_ok` for a denied UPDATE — but RLS-denied UPDATE (no UPDATE policy) silently affects 0 rows rather than raising; corrected to assert the row is unchanged (`plan` 7→8, `cca50cc`); re-ran **CI all required green**. NEXT: set **Phase 10 (first-class agent-to-agent interaction)** per the dependency graph (9 → 10 → 11 registry). PRs #4→#14 remain open for the owner to merge bottom-up.
...
- **[Phase 11 — Med, security follow-up: `agents.system_prompt` is readable by any authenticated user]** The pre-existing `agents_select` RLS policy is `FOR SELECT USING (auth.uid() IS NOT NULL)` — all columns of all agent rows, including `system_prompt`, are readable by any logged-in account via the browser client. Phase 11 now lets users author `system_prompt`, so this is worsened. Mitigated immediately with a UI caption ("Visible to room members — don't put secrets in the system prompt") and the server routes already return a safe column subset. **Proper fix (deferred — needs live-DB verification):** restrict the policy to non-sensitive columns via a view, or `REVOKE SELECT (system_prompt, tool_permissions) … FROM authenticated, anon; GRANT SELECT (safe cols) …`, with a pgTAP test. NOT done unattended because PostgREST `select=*` interacts with column grants and a wrong grant could break legitimate client reads (AgentsPanel/ComposeBox select specific columns today, but this can't be confirmed against a live DB headless). security-auditor rated it **Medium** (system_prompt is persona text, not a credential), and both critics returned **VERDICT: PASS, 0 Critical/High** — so it does not block Phase 11.
  - **2026-05-31 (re-invocation — static de-risking + ready-to-run recipe; NOT applied unattended).** I exhaustively traced every read of `public.agents`: the **only** browser (`authenticated`) read is `AgentsPanel.tsx:61` — `agents!inner(id, name, slug, capabilities, is_active, created_by_user_id)` (explicit columns, no `*`, no sensitive cols). All server reads (`api/agents/route.ts`, `[agentId]/route.ts`, `members/route.ts` embed, `health.ts`) select explicit non-sensitive columns and the members embed uses the **service_role** client (unaffected by `authenticated` grants). **`agents` is NOT in the realtime publication** (only messages/agent_runs/tool_calls/files/pinned_items/agent_memory). ⇒ No current code path reads `system_prompt`/`tool_permissions` from the browser, so a column-grant restriction is **safe against today's code**. **Exact recipe (additive migration + pgTAP):**
    ```sql
    -- Postgres GOTCHA: a table-level GRANT SELECT overrides a column-level REVOKE,
    -- so you MUST drop the table grant and re-grant the safe columns explicitly.
    REVOKE SELECT ON public.agents FROM anon, authenticated;
    GRANT  SELECT (id, name, slug, avatar_url, provider, adapter_type, model,
                   reply_policy, is_active, created_by_user_id, capabilities,
                   created_at, updated_at)
           ON public.agents TO authenticated;
    -- anon reads nothing (agents_select RLS requires auth.uid()); grant omitted for anon.
    ```
    pgTAP: assert `authenticated` **cannot** select `system_prompt`/`tool_permissions` and **can** select the safe set. **Why still owner-gated (not done headless):** (1) the table-vs-column footgun — forget one safe column and that read breaks while pgTAP still passes; (2) pgTAP verifies grant *state*, not PostgREST **embed resolution** (`agents!inner(...)`) — only a live authed app proves AgentsPanel/members still resolve; (3) it's a **maintenance footgun** (future columns won't inherit the grant → reads of them break until granted) which is an ADR-level call. Both critics PASS without it. **Owner action:** run the recipe above on a branch, verify AgentsPanel + room-members UI against the live authed app, add the pgTAP test, then merge. ~15 min with a seeded app.
...
- Acceptance criteria (testable):
  ...
  - [x] **Parser** (`slash-commands.ts`) is registry-driven: detects a leading `/command`, rejects unknown/over-privileged with a friendly message, coexists with `@mention`/`/discuss`/Phase 9–10 commands (tests pass). No command bypasses RLS or the tool-approval flow.
...
## 2026-05-31 — GOAL: Phase 10 — First-class agent-to-agent interaction — **DONE ✅ (PR #15, CI green)**
- Phase: 10 (Agent-to-agent). Branch: `harden/p10-agent-to-agent` (stacked off p9 HEAD). PR: **#15 (→ `main`)**.
- Design source: `04_HERMES_CAPABILITIES.md` §Phase 10 (owner pointed the loop at this spec → approved design; builds on the existing loop guards `allow_agent_to_agent`/`max_agent_rounds`/`max_agent_hops`/`reply_policy`/`/discuss`/tag-turns).
- Iteration budget: 12. State: **DONE** — judge-gated: all criteria met w/ evidence; security-auditor **PASS** + code-reviewer FAIL→**PASS** (self-healed 2 High: wired the hand-off engine end-to-end via adapter stdout envelopes + corrected the docs); local gate green; **CI green on PR #15** (`verify`/`build-images`/`Playwright`/`codeql`/`CodeQL`/`rls`/`secret-scan` PASS; `audit` allowed-red per D3).
- Acceptance criteria (testable; from `04_HERMES_CAPABILITIES.md` §Phase 10 + DoD §"Hermes capabilities" agent-to-agent box):
  - [x] **Roster in context:** migration adds `agents.capabilities`; `ContextPacketV1.roster` injects the OTHER active, unmuted, reply-enabled room agents (`name`/`slug`/`capabilities`), rendered as DATA in the adapter prompt. `format-roster.test.ts` + the build excludes self/muted/reply-disabled.
  - [x] **Hand-off protocol:** `handoff_requested` `AgentEvent` (now **emitted** by the subprocess adapters from a stdout envelope) → `agents/handoff.ts` resolves the peer + creates a **targeted** run under `allow_agent_to_agent`/`max_agent_hops`/`max_agent_rounds` **plus cycle detection**; invalid/blocked never crashes the run. `handoff.test.ts` (9) covers success/hop-cap/round-cap/cycle(A→B→A)/self/dup/disabled; `run-worker.test.ts` integration proves the wiring end-to-end.
  - [x] **Loop-guard hardening:** deterministic termination proven (cycle + hop-cap + round-cap tests; round_index strictly increases, never reset on agent paths); a visible **"Deliberation ended (…)"** system message is posted on cap/cycle. Per-run hand-off cap bounds fan-out.
  - [x] **`/handoff @agent <task>`** (rewritten to a targeted mention message) and **`/agents`** (AgentsPanel: agents + capabilities + active run counts) via the slash parser; coexists with `@mention`/`/discuss`/`/remember`/`/recall` (tests pass); parser tests added.
  - [x] **Docs:** `docs/ARCHITECTURE.md` documents the hand-off protocol (agent-emitted vs user `/handoff`), discussion phases/tag-turns, and the loop-guard math.
  - [x] Critique gate: **security-auditor PASS + code-reviewer PASS** (after self-heal), 0 open Critical/High → `docs/reviews/2026-05-31-phase10-agent-to-agent.md`. Local gate green (bridge 130 / web 133; build 0 Edge warnings) + **CI green on PR #15**.

Judge rule: DONE only when every criterion is checked with linked evidence and no Critical/High is open. The loop-termination (cycle/hop-cap) test is a hard gate — an agent-to-agent feature must provably never infinite-loop.

---

## 2026-05-31 — GOAL: Phase 9 — In-product agent memory (Hermes-style, Postgres FTS) — **DONE ✅ (PR #14, CI green)**
- Phase: 9 (Agent memory). Branch: `harden/p9-agent-memory` (stacked off p6-edge-logger HEAD `77a2795`). PR: **#14 (→ `main`)**.
...
- Iteration budget: 12. State: **DONE** — judge-gated: all criteria met w/ evidence; security-auditor + code-reviewer **PASS** (0 Critical/High); local gate green; **CI green on PR #14** (`verify`/`build-images`/`Playwright`/`codeql`/`CodeQL`/`rls`/`secret-scan` PASS; `audit` allowed-red per D3). One self-heal: pgTAP UPDATE assertion corrected (RLS-denied UPDATE is a silent no-op, not an error) → `cca50cc`.
...
8. **README badges (Phase 8 DoD)** — CI + MIT-license badges, then a release badge once
   ...
   CI badge would 404. Trivial once #4 + #11 land — add the badge block atop `README.md`
   (`actions/workflows/ci.yml/badge.svg` + a shields.io MIT badge); release badge at tag time.
...
Branch `feat/collab-and-output-hardening` (off `feat/product-validation-v1`). `main`+`v1.0.0` untouched; not pushed.
  ...
  - [x] **PR #41 OPEN + CI GREEN** (base `feat/product-validation-v1`): all required checks pass (verify · rls · build-images · Playwright e2e · CodeQL); `audit` allowed-red per D3. Self-healed 3 CI-only issues (prettier format, knip unused-exports, 2 CodeQL ReDoS HIGHs in parseTaskList). Critique Critical/High-free. https://github.com/neric-joel/Whatsapp-Agents/pull/41
