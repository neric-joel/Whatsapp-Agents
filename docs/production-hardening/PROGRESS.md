...
## 2026-05-31 — GOAL: Phase 11 — In-product slash commands + command registry + user-created agents — **ACTIVE**
...
- Iteration budget: 14 (two workstreams: command registry/RBAC + create/edit/disable agents). State: **ACTIVE**.
- Acceptance criteria (testable):
  - [ ] **Central command registry** in `packages/shared`: each command = `{ name, description, argsSpec, minRole, surface }` (single source of truth). Both the parser and the API read from it. Unit test enumerates the v1 set.
  - [ ] **RBAC tiers** on the existing `MemberRole` (owner > admin > member): `/help` + `/commands` always allowed; admin-only commands (e.g. `/reset`) require admin+. Server-side enforcement — a `member` calling an admin command is rejected (test proves it; not just hidden in the UI).
  - [ ] **Parser** detects a leading `/command` and validates args via the registry's `argsSpec`; unknown/over-privileged commands get a friendly rejection. Coexists with `@mention`, `/discuss`, and the Phase 9/10 commands (existing tests pass). No command bypasses RLS or the tool-approval flow.
  - [ ] **v1 command set** wired: `/help`, `/commands`, `/discuss` (exists), `/remember`, `/recall` (Phase 9), `/handoff`, `/agents` (Phase 10), `/pin`, `/reset` (admin+, clears the room's rolling agent context). `/help` lists exactly the caller's allowed commands.
  - [ ] **User-created agents:** `POST/PATCH/DELETE /api/agents` with `createAgentSchema`/`updateAgentSchema` (name, slug, avatar_url, provider, adapter_type, model, system_prompt, capabilities, reply_policy, tool_permissions); `created_by_user_id = auth.uid()`; **admin+ gated server-side**; a "Create agent" UI surface; created agents are addable as `room_members` and appear in AgentsPanel. Non-admin create/edit is rejected (test proves it).
  - [ ] **⚠ Security coupling (MANDATORY):** a user-set `system_prompt` must reach the CLI **only via stdin, never argv** (the Phase-1 subprocess-injection invariant) — confirm + test that a created agent's system_prompt cannot inject shell/argv. security-auditor critique is MANDATORY for this phase.
  - [ ] Critique gate: **security-auditor (MANDATORY) + code-reviewer** → PASS, 0 open Critical/High → `docs/reviews/2026-05-31-phase11-commands-user-agents.md`. Local gate green + **CI green on the PR**.

Judge rule: DONE only when every criterion is checked with linked evidence and no Critical/High is open. Server-side RBAC enforcement + the user-`system_prompt` injection-safety test are hard gates.
