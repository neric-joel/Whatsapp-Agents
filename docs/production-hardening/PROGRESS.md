...
## 2026-05-30 — GOAL: Phase 6 — Observability, reliability & error handling — **ACTIVE**
...
- Acceptance criteria (testable; from 01_HARDENING_PLAN §Phase 6 + DoD):
  - [ ] Structured logging (JSON: level, timestamp, worker_id, run/correlation IDs) in BOTH bridge + web API; secrets/PII redacted; stray `console.*` replaced. Unit test proves redaction + shape.
  - [ ] Health/readiness: `/api/health` reflects reality; bridge liveness observable (heartbeat/health signal); stale-run recovery documented.
  - [ ] Error tracking wired behind config — opt-in, **no-op without a DSN** (unit test proves no-op when unset).
  - [ ] Reliability: run state machine can't get stuck (claimed→running→completed/failed/cancelled); cancellation truly kills the child; graceful handling of CLI crash/timeout, Supabase error, backpressure. Tests for induced child-crash / bad-agent-output / DB-error → clean failed state, no hang, no lost run.
  - [ ] Minimal runtime metrics (runs queued/active/failed, latency) exposed for scraping (or a documented signal, since the bridge has no HTTP server).
  - [ ] Critique gate (Adversarial chaos Critic — "what still hangs or loses a run?") PASS → `docs/reviews/`; all checks green (typecheck/lint/test/build + CI); no Critical/High.
