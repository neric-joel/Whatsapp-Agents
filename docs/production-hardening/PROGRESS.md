...
## 2026-05-31 — GOAL: Phase 10 — First-class agent-to-agent interaction — **ACTIVE**
- Phase: 10 (Agent-to-agent). Branch: `harden/p10-agent-to-agent` (stacked off p9 HEAD). PR → `main` (clean CI per the stacked-dirty-merge note).
...
- Acceptance criteria (testable; from `04_HERMES_CAPABILITIES.md` §Phase 10 + DoD §"Hermes capabilities" agent-to-agent box):
  - [ ] **Roster in context:** additive migration adds `agents.capabilities` (short text blurb); `ContextPacketV1.roster` injects the OTHER active room agents (`name`, `slug`, `capabilities`) so each agent can address peers deliberately. Type added to `packages/shared`; bridge populates it in `build-context-packet.ts`; rendered as DATA in the adapter prompt. Unit test proves the roster excludes the acting agent + muted/inactive agents.
  - [ ] **Hand-off protocol:** new `AgentEvent` variant `{ type:'handoff_requested'; to_agent_slug; reason; payload? }`. The bridge resolves the slug to a room agent and creates a **targeted** `agent_run` for it, subject to the existing guards (`allow_agent_to_agent`, `max_agent_hops`, `max_agent_rounds`) **plus cycle detection** on the hand-off chain. Invalid/unknown slug or a guard/cycle block is logged + surfaced, never crashes the run. Bridge unit tests cover: successful hand-off creates one targeted run; hop-cap/round-cap blocks it; a cycle (A→B→A) is detected and stopped.
  - [ ] **Loop-guard hardening:** a deterministic cap terminates the chain (no infinite loops, proven by a test) and a visible **"deliberation ended (hop limit)"** system message is posted when the cap is hit.
  - [ ] **`/handoff @agent <task>`** user surface (parser + API → targeted run) and **`/agents`** (lists room agents + capabilities + active runs; reuses the panel/`agent_runs` pattern). Parser coexists with `@mention`/`/discuss`/`/remember`/`/recall` (existing tests pass); validation + parser tests added.
  - [ ] **Docs:** `docs/ARCHITECTURE.md` documents the hand-off protocol — discussion phases, tag-turns, hand-offs, and the loop-guard math (hops × rounds).
  - [ ] Critique gate: **security-auditor (MANDATORY for Phase 10) + code-reviewer** → PASS, 0 open Critical/High → `docs/reviews/2026-05-31-phase10-agent-to-agent.md`. Local gate green + **CI green on the PR** (`verify`/`build-images`/Playwright/`rls`/`secret-scan`/`codeql`; `audit` allowed-red per D3).
