...
test('context message limit defaults and clamps to a safe range', () => {
  ...
  assert.equal(readContextMessageLimit({ AGENTROOM_CONTEXT_MESSAGE_LIMIT: 'bad' }), DEFAULT_CONTEXT_MESSAGE_LIMIT)
})
...
test('context message char limit defaults and clamps to a safe range', () => {
  ...
  assert.equal(readContextMessageMaxChars({ AGENTROOM_CONTEXT_MESSAGE_MAX_CHARS: 'bad' }), DEFAULT_CONTEXT_MESSAGE_MAX_CHARS)
})

test('trims oversized context message content with an explicit marker', () => {
  assert.equal(trimContextContent('short', 10), 'short')
  assert.equal(trimContextContent('abcdefghijklmnopqrstuvwxyz', 10), 'abcdefghij\n[...truncated 16 chars]')
})
